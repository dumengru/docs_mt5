...
protected:
   ...
   //--- 添加过滤器
   CArrayObj         m_filters;        // 过滤器数组(最大64)
   //--- Adjusted parameters
   double            m_weight;         // 信号权重
   int               m_patterns_usage; // 使用位掩码的信号模型
   int               m_general;        // index of the "main" signal (-1 - no)
   long              m_ignore;         // "忽略"附加过滤器的位掩码
   long              m_invert;         // "反向"附加过滤器的位掩码
   int               m_threshold_open; // 开仓阈值
   int               m_threshold_close;// 平仓阈值
   double            m_price_level;    // level of placing a pending orders relatively to the base price
   double            m_stop_level;     // 止损价
   double            m_take_level;     // 止盈价
   int               m_expiration;     // 订单过期时间
   double            m_direction;      // 方向
