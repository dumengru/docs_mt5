...
enum ENUM_ALIGNMENT
  {
   ...
   ALIGNMENT_TOP         = 2,   // 上对齐
   ALIGNMENT_RIGHT       = 4,   // 右对齐
   ALIGNMENT_BOTTOM      = 8    // 下对齐
  };
//--- macro
...
protected:
   //--- colors
   uint              m_color_background;     // 背景色
   uint              m_color_border;         // 边框色
   uint              m_color_text;           // 文字色
   uint              m_color_grid;           // 网格色
   //--- adjusted parameters
   uint              m_max_data;             // 最大数据量
   uint              m_max_descr_len;        // 最大描述符长度
   uint              m_allowed_show_flags;
   uint              m_show_flags;           // 可视标识
   ENUM_ALIGNMENT    m_legend_alignment;     // 对齐方式
   uint              m_threshold_drawing;
   bool              m_accumulative;         // 序列值累计标识
   //--- parameters for scales and grid
   double            m_v_scale_min;          // 坐标最小值
   double            m_v_scale_max;          // 坐标最大值
   uint              m_num_grid;
   int               m_scale_digits;
   //--- data
   int               m_data_offset;
   uint              m_data_total;  // 数据总量
   CArray           *m_data;        // 保存数据的数组
   CArrayInt         m_colors;      // 保存颜色的数组
   CArrayString      m_descriptors; // 保存标签的数组
   //---
   CArrayInt         m_index;
   uint              m_index_size;
   double            m_sum;
