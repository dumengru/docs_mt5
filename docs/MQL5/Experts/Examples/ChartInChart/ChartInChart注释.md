...
```cpp
input color TextColor=White;    // 文字颜色
input color BGColor=SteelBlue;  // 背景色
input int   XPosition=10;       // X轴坐标
input int   YPosition=10;       // Y轴坐标
input int   XSize=450;          // X轴长度
input int   YSize=300;          // Y轴长度
//--- variables
int xsize=450;
int ysize=300;
int xdist=10;
...
//--- Symbol: 颜色, 背景色, X轴, Y轴, 宽,高, 字体, 内容, 字号, 可用性
...
//--- Period: 颜色, 背景色, X, Y, 宽, 高, 字体, 内容, 字号, 可用性
...
//--- Dates: 颜色, 背景色, X, Y, 宽, 高, 内容, 字体, 字号, 按钮状态, 可用性
...
//--- Chart 品种, 周期, X, Y, 宽, 高, 显示时间? 显示价格? 可用性, 放大倍数
   ObjectSetString(0,"PIPChart",OBJPROP_SYMBOL,curr_symbol);
   ObjectSetInteger(0,"PIPChart",OBJPROP_PERIOD,curr_period);
