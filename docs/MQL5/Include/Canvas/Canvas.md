...
private:
   ...
   static uint       m_default_colors[9];      // 默认颜色

protected:
   long              m_chart_id;               // 图表ID
   string            m_objname;                // 物件名称
   ENUM_OBJECT       m_objtype;                // 物件类型
   string            m_rcname;                 // 资源名称
   int               m_width;                  // 图形宽度
   int               m_height;                 // 图形高度
   ENUM_COLOR_FORMAT m_format;                 // 颜色处理方案
   //--- 文本属性
   string            m_fontname;               // 字体名称
   int               m_fontsize;               // 字号
   uint              m_fontflags;              // 字体标识
   uint              m_fontangle;              // 字体角度
   //--- data
   uint              m_pixels[];               // 像素数组
