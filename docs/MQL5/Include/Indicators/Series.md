...
class CSeries : public CArrayObj
  {
protected:
   string            m_name;             // 指标名称(子类实现)
   int               m_buffers_total;    // 缓冲区数量
   int               m_buffer_size;      // 缓冲区大小
   int               m_timeframe_flags;  // 时间周期标志(不同时间周期通过左移运算符实现)
   string            m_symbol;           // 品种
   ENUM_TIMEFRAMES   m_period;           // 周期
   bool              m_refresh_current;  // 是否刷新
   //--- 
...
定义Double数据缓冲区, 只是简单包装 CArrayDouble
