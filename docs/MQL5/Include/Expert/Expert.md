...
enum ENUM_TRADE_EVENTS
  {
   TRADE_EVENT_NO_EVENT              =0,         // 无事件
   TRADE_EVENT_POSITION_OPEN         =0x1,       // 开仓事件
   TRADE_EVENT_POSITION_VOLUME_CHANGE=0x2,       // 持仓量改变
   TRADE_EVENT_POSITION_MODIFY       =0x4,       // 持仓单变动
   TRADE_EVENT_POSITION_CLOSE        =0x8,       // 平仓事件
   TRADE_EVENT_POSITION_STOP_TAKE    =0x10,      // 触发持仓
   TRADE_EVENT_ORDER_PLACE           =0x20,      // 放置订单
   TRADE_EVENT_ORDER_MODIFY          =0x40,      // 修改订单
   TRADE_EVENT_ORDER_DELETE          =0x80,      // 删除订单
   TRADE_EVENT_ORDER_TRIGGER         =0x100      // 触发订单
  };
//+------------------------------------------------------------------+
//| Macro definitions.                                               |
//+------------------------------------------------------------------+
...
class CExpert : public CExpertBase
  {
protected:
   int               m_period_flags;             // 时间框架标志
   int               m_max_orders;               // 最大订单量 (包括持仓)
   MqlDateTime       m_last_tick_time;           // 最新Tick时间
   datetime          m_expiration;               // 订单到期时间
   //--- 历史信息
   int               m_pos_tot;                  // 持仓数量
   int               m_deal_tot;                 // 成交数量
   int               m_ord_tot;                  // 挂单数量
   int               m_hist_ord_tot;             // 成交订单数量
   datetime          m_beg_date;                 // 历史开始日期
   //---
   int               m_waiting_event;            // 交易事件
   //--- trading objects
   CExpertTrade     *m_trade;                    // 交易对象
   CExpertSignal    *m_signal;                   // 信号对象
   CExpertMoney     *m_money;                    // 资金管理对象
   CExpertTrailing  *m_trailing;                 // 尾随停止对象
   bool              m_check_volume;             // 订单发送前检查交易量
   //--- indicators
   CIndicators       m_indicators;               // 指标集合
   //--- market objects
   CPositionInfo     m_position;                 // 持仓对象
   COrderInfo        m_order;                    // 订单对象
   //--- 事件处理器
   bool              m_on_tick_process;          // 触发OnTick(默认true)
   bool              m_on_trade_process;         // 触发OnTrade(默认false)
   bool              m_on_timer_process;         // 触发OnTimer(默认false)
   bool              m_on_chart_event_process;   // 触发OnChartEvent(默认false)
   bool              m_on_book_event_process;    // 触发OnBookEvent(默认false)
...
//--- 3. 设置参数
   ...
   if(every_tick)
      TimeframeAdd(WRONG_VALUE);            // 添加全部周期
   else
      TimeframeAdd(period);                 // 只添加特定周期
//--- 如果品种精度是3或5, 调整点数*10
...
//--- initialization for working with trade history
   PrepareHistoryDate();   // 设置起始时间
   HistoryPoint();         // 获取所有挂单和持仓
//--- 初始化成功, 更新状态
